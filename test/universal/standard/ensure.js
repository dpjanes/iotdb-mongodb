/**
 *  test/universal/standard/ensure.js
 *
 *  David Janes
 *  IOTDB
 *  2020-01-20
 *
 *  Copyright (2013-2020) David P. Janes
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */

"use strict"

const _ = require("iotdb-helpers")

const assert = require("assert")

const _util = require("../../_util")

const db = require("./_db")

describe("universal/create", function() {
    let self = {}

    before(function(done) {
        _.promise(self)
            .then(_util.initialize)
            .then(_util.load)
            .make(sd => {
                self = sd;
            })
            .end(done)
    })

    it("works", function(done) {
        _.promise(self)
            .add("movie", {
                "title": "Avengers : Infinity War",
                "year": 2018,
            })
            .then(db.movie.ensure)
            .then(db.movie.list.year.p(2018))
            .make(sd => {
                assert.deepEqual(sd.movies.length, 1)
                assert.deepEqual(sd.movies[0].year, 2018)
                assert.deepEqual(sd.movies[0].title, "Avengers : Infinity War")
            })
            .end(done)
    })
    it("repeatedly", function(done) {
        _.promise(self)
            .add("movie", {
                "title": "Avengers : Infinity War",
                "year": 2018,
            })
            .then(db.movie.ensure)
            .then(db.movie.list.year.p(2018))
            .make(sd => {
                assert.deepEqual(sd.movies.length, 1)
                sd.original = sd.movies[0]
            })

            .add("movie", {
                "title": "Avengers : Infinity War",
                "year": 2018,
                "description": "bla bla"
            })
            .then(db.movie.ensure)
            .then(db.movie.list.year.p(2018))
            .make(sd => {
                assert.deepEqual(sd.movies.length, 1)
                assert.deepEqual(sd.movies[0].description, "bla bla")
            })

            .end(done)
    })
    it("parameterized", function(done) {
        _.promise(self)
            .then(db.movie.create.p({
                "title": "Avengers : Endgame",
                "year": 2019,
            }))
            .make(sd => {
                assert.ok(sd.movie)
                assert.deepEqual(sd.movie.year, 2019)
                assert.deepEqual(sd.movie.title, "Avengers : Endgame")
            })

            .add("movie", null)
            .then(db.movie.by.title.p("Avengers : Endgame"))
            .make(sd => {
                assert.ok(sd.movie)
                assert.deepEqual(sd.movie.year, 2019)
                assert.deepEqual(sd.movie.title, "Avengers : Endgame")
            })
            .end(done)
    })
    it("fails with missing keys", function(done) {
        _.promise(self)
            .then(db.movie.create.p({}))
            .then(_util.auto_fail(done))
            .catch(_util.ok_error(done))
    })
})
