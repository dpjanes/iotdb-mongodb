...
const remove = (_self, done) => {
    ...
    const options = self.options || exports.remove.DEFAULT;

    console.log("HERE:XXX", query)

    self.mongo_collection.remove(query, options, (error, mongo_result) => {
        if (error) {
            return done(error)
        }
 ...
 *  API
 */
exports.remove = Q.denodeify(remove)
