...
/**
 *  Requires: self.mongodb$collection, self.query
...
const find_one = _.promise.make((self, done) => {
    ...
    self.mongodb$collection.findOne(query, find_one_in, (error, mongodb$result) => {
        if (error) {
        ...
        if (mongodb$result) {
            self.json = util.safe_ids(mongodb$result);
        }
