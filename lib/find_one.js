...
const find_one = (_self, done) => {
    ...
    const options = self.options || exports.find_one.DEFAULT;

    self.mongo_collection.findOne(query, options).toArray((error, mongo_result) => {
        if (error) {
            return done(error)
        }

        mongo_result.forEach(d => d._id = d._id.toString())

        self.jsons = mongo_result
 ...
 *  API
 */
exports.find_one = Q.denodeify(find_one)
