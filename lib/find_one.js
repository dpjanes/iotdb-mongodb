...
const find_one = (_self, done) => {
    ...
    const find_one_options = self.find_one_options || exports.find_one.DEFAULT;

    self.mongo_collection.findOne(query, find_one_options).toArray((error, mongo_result) => {
        if (error) {
            return done(error)
        }
 ...
 *  API
 */
exports.find_one = _.promise.denodeify(find_one)
