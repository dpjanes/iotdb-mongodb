...
const insert = (_self, done) => {
    ...
    const insert_options = self.insert_options || { w: 1 };

    self.mongo_collection.insert(json, insert_options, (error, mongo_result) => {
        if (error) {
            return done(error)
        }
        ...
        self.mongo_result = util.safe_ids(mongo_result);
 ...
 *  API
 */
exports.insert = _.promise.denodeify(insert)
